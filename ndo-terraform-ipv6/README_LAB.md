...
>
> | File | Auto-loaded? | Holds | Pass via |
> |---|---|---|---|
> | `terraform.tfvars` | yes | active credentials + NDO URL only | n/a |
> | `lab.tfvars` | no | env config: `vrf_template_name`, `mso_domain`, `mso_platform` | `-var-file=lab.tfvars` |
> | `prod.tfvars` | no | env config: `vrf_template_name` (production value) | `-var-file=prod.tfvars` |
>
> All three files match `*.tfvars` in `.gitignore`, so none of them get committed. Do NOT add credentials to `lab.tfvars` or `prod.tfvars` — they would override `terraform.tfvars` at plan time and break authentication.
...
| Flag | Why |
|---|---|
| `-refresh=false` | NDO API throttles aggressively; a full state refresh on 360+ resources triggers session timeouts and provider crashes mid-plan. Trust the existing state and only act on `.tf` changes. |
| `-parallelism=3` | Empirically the sweet spot. Higher values trigger NDO session drops and provider crashes; lower values (1–2) also fail or hang intermittently in this stack. Don't change without a reason. |
...
5. `../scripts/deploy_bindings.py` (or equivalent) — push static port bindings that NAC YAML doesn't model.
...
| `terraform.tfvars` | gitignored | Your real lab credentials |
| `lab.tfvars` | 8 | Placeholder/template; not auto-loaded unless you pass `-var-file=lab.tfvars` |

### Deferred — re-enable after bindings

Three files are suffixed `.disabled` **on purpose**. They must be applied after `bds_epgs.tf` is in NDO and after static port bindings have been pushed (workflow step 5), and they form a strict ordered chain. To activate one, rename it from `*.tf.disabled` to `*.tf`; to deactivate, rename back. Do not just delete them.

| Stage | File | Bytes | Provider | Manages | Hard prerequisite |
|---|---|---|---|---|---|
| 6a | `l3outs_ndo.tf.disabled` | ~36 KB | `mso` | Site-local L3Outs (`L3Out-RCC-E-G`, `L3Out-RCC-E-K`) and External EPGs (`ExtEPG-RCC-E-G/K`) in the `G-Specific_Only` / `K-Specific_Only` templates | `bds_epgs.tf` already applied; bindings pushed |
| 6b | `l3outs_apic.tf.disabled` | ~19 KB | `aci` (per-site aliases `aci.apic_g`, `aci.apic_k`) | OSPF policies, logical node/interface profiles, IPv6 path attachments directly on each APIC | Stage 6a applied **and** NDO has finished pushing the L3Outs to **both** APICs (verify in APIC UI before proceeding) |
| 6c | `vlans_apic.tf.disabled` | ~7 KB | `aci` (reuses `aci.apic_g`, `aci.apic_k` from stage 6b) | Individual VLAN entries under existing `Pool_VLAN_All_Combined` on both APICs | Stage 6b applied (provides the per-site `aci` providers and APIC variables) |

**Activation sequence (one stage at a time, plan and verify between each):**

1. **Stage 6a — NDO L3Outs.** Rename, plan, apply, then deploy templates from the NDO UI:
   ```bash
   mv l3outs_ndo.tf.disabled l3outs_ndo.tf
   terraform plan -var-file=lab.tfvars -refresh=false -parallelism=3 -out=l3outs_ndo.tfplan
   terraform apply l3outs_ndo.tfplan
   # then deploy the affected templates from the NDO UI and confirm both APICs show
   # L3Out-RCC-E-G / L3Out-RCC-E-K under tn-EUR before doing stage 6b
   ```
2. **Stage 6b — APIC L3Out details.** Two edits before plan: rename, **and** remove the `/* ... */` wrapping the resources (the file ships with them block-commented to prevent accidental apply against an APIC that doesn't yet have the parent L3Out). Also set APIC credentials — this stage introduces new variables (`apic_g_url`, `apic_k_url`, `apic_username`, `apic_password`, `tenant_name`, `l3out_vlan`, OSPF settings) declared inside the file with placeholder defaults. Override them in `terraform.tfvars` (keep `apic_password` out of git):
   ```bash
   ...
   # then add APIC creds to terraform.tfvars, e.g.:
   #   apic_g_url    = "https://<aedcg-apic>"
   #   apic_k_url    = "https://<aedck-apic>"
   #   apic_username = "<svc-acct>"
   #   apic_password = "<set via env or vault, not committed>"
   terraform plan -var-file=lab.tfvars -refresh=false -parallelism=3 -out=l3outs_apic.tfplan
   terraform apply l3outs_apic.tfplan
   ```
3. **Stage 6c — APIC VLAN entries.**
   ```bash
   ...
   terraform apply vlans_apic.tfplan
   ```
...
- `l3outs_apic.tf.disabled.old` — earlier revision of stage 6b kept for diff/recovery; do **not** rename this file. If you need something out of it, diff against `l3outs_apic.tf.disabled`.
...
- **Static port bindings** — handled outside Terraform by `generate_ipv6_bindings3.py`. The script's defaults are Design A port-channel bindings only (PC_FI_A/B on AEDCG 152/153, AEDCK 119/191); two opt-in flags layer on:
