#!/usr/bin/env python3
"""
NDO static-port-binding deployer for the IPv4 redesign (schema AEDCE-IPv4).

This is a fork of /Users/johbarbe/DC/NXOS/n5k/deploy_bindings_python_v2_prod.py
with default schema set to AEDCE-IPv4 and the production NDO host removed
...
identical:
  ...
  * Multi-template auto-resolve: looks up each EPG's template name in NDO
    ...
    For the IPv4 redesign that name is Tenant_EUR_IPv4.
...
Bindings JSON shape -- see scripts/bindings.example.json. Minimum:
  {
    ...
    "schema_name":   "AEDCE-IPv4",
    "static_port_bindings": [
      {
        "site":      "AEDCG",
        "epg_name":  "EPG-WEB-SVR",
        "vlan":      100,
...
Operational sequence (full IPv4 cutover):
  1. ndo/ Terraform root creates schema AEDCE-IPv4 / template Tenant_EUR_IPv4
...
# IPv4 redesign defaults. NDO_HOST is intentionally empty so a stale
