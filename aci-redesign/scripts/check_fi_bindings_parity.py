...
The IPv4 redesign deploys two coupled artifacts:

    1. Tenant tree (VRFs, BDs, EPGs, contracts) -- pushed via Terraform
       from ``aci-redesign/data/nac-ndo/schema-aedce-ipv4.nac.yaml``.
...
The manifest's ``mode`` field selects the parity rule:
    ...
    * ``"physical-only"`` (Approach 1, lab): only EPGs that lack a VMM
      domain in the schema (today: EPG-LB, EPG-LMR, EPG-VHOST-MGMT) must
      appear. VMM-bound EPGs are intentionally skipped because lab cutover
      relies on VMM dynamic VLAN learning. Manifest entries for VMM-bound
      EPGs in this mode are reported as warnings.
...
def main() -> int:
    ...
    p = argparse.ArgumentParser(
        description=(
            ...
            "aci-redesign/data/nac-ndo/schema-aedce-ipv4.nac.yaml. "
