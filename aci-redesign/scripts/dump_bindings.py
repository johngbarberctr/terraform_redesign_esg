#!/usr/bin/env python3
"""
NDO static-port-binding dumper for the IPv4 redesign.
...
Why this exists:
  * The IPv4 redesign (schema AEDCE-IPv4 / AppProf-NetCentric + AppProf-DMZ)
    has the SAME 39 EPG names as the IPv6 redesign in AEDCE / AppProf-RCC.
  * Dual-stack hosts land on the same physical interfaces, so the IPv6
    binding paths (leaf, port, vPC) are reusable for IPv4.
  * The VLANs are NOT reusable: IPv6 uses static 3001-3500, IPv4 uses VMM
...
ANP routing:
  --target-netcentric-anp AppProf-NetCentric
  --target-dmz-anp        AppProf-DMZ
  --dmz-epgs "EPG-D64-PROXY,EPG-FWEB-PROXY,EPG-RWEB-PROXY"
  Everything not in --dmz-epgs lands under the netcentric ANP.

Validation:
  If --target-schema is set (default AEDCE-IPv4), the script also pulls
...
DEFAULT_TARGET_SCHEMA = "AEDCE-IPv4"
DEFAULT_NETCENTRIC    = "AppProf-NetCentric"
DEFAULT_DMZ_ANP       = "AppProf-DMZ"
