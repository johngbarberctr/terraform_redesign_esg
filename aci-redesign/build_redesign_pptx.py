...
def title_slide():
    ...
    add_text(s, Inches(0.8), Inches(6.8), Inches(11.8), Inches(0.3),
             "Prepared from IPv4_REDESIGN_OVERVIEW.md and aci-redesign/README.md",
...
def agenda_slide():
    ...
    items = [
        ...
        ("3  Key design decisions", "Why 2 VRFs, why L2 'DMZ' stays in VRF-EUR, no IP changes"),
...
def executive_summary_slide():
    ...
    add_bullets(
        ...
        [
            "Collapse 11 legacy VRFs into 2 routing domains: VRF-EUR (internal) and VRF-DMZ (proxy).",
...
def target_state_slide():
    s = prs.slides.add_slide(BLANK)
    page_header(s, "Target State - 2-VRF Redesign",
                "VRF-EUR + VRF-DMZ, 39 BDs, 39 EPGs, 2 ESGs, vzAny permit-all start")
    headers = ["Attribute", "Target"]
    rows = [
        ["Tenant", "EUR (unchanged)"],
        ["VRFs", "2  -  VRF-EUR (internal) + VRF-DMZ (proxy segments)"],
        ["Bridge Domains",
         "39 descriptive  -  BD-AD, BD-APP-SVR, BD-CFG-MGMT, ..."],
        ["EPGs", "39 (1:1 with BDs)  -  EPG-AD, EPG-APP-SVR, ..."],
        ["Contracts",
         "vzAny permit-all (initial) - tightened via ESGs later"],
        ["ESGs",
         "ESG-All-Internal-EPGs (36) + ESG-All-DMZ-EPGs (3)  -  "
         "deferred (nac-ndo ~> 1.2.0 lacks ESG selectors)"],
        ["App Profiles",
         "AppProf-NetCentric, AppProf-DMZ"],
        ["IPv6 (VRF-RCC)",
         "Unchanged  -  managed separately in ndo-terraform-nac"],
    ]
    ...
    panel(
        ...
        "Why this shape",
        [
            "VRF-EUR consolidates all internal IPv4 traffic into a single routing domain.",
            "VRF-DMZ keeps proxy segments (139.139.x.x) routing-isolated from internal.",
...
def architecture_diagram_slide():
    ...
    add_text(s, tx + Inches(4), ty, tw - Inches(4.2), Inches(0.4),
             "Filters: Any  |  Contracts: Any_VRF-EUR, Any_VRF-DMZ",
    ...
    add_text(s, ex + Inches(0.15), ey, ew, Inches(0.45),
             "VRF-EUR  -  Internal IPv4   |   vzAny: Any_VRF-EUR (permit-all)",
    ...
    panel(s, bdx + appw + Inches(0.1), appy, appw, Inches(1.25),
          "ESG-All-Internal-EPGs", [
              "Selects all 36 internal EPGs",
              "Provider + consumer of Any_VRF-EUR",
              "Classification ready for Phase 3",
          ], accent=GREEN)
    ...
    add_text(s, dx + Inches(0.15), dy, dw, Inches(0.45),
             "VRF-DMZ  -  Proxy", size=13, bold=True, color=WHITE,
             anchor=MSO_ANCHOR.MIDDLE)
    ...
    panel(s, dx + Inches(0.15), dy + Inches(3.35),
          dw - Inches(0.3), Inches(1.25),
          "ESG-All-DMZ-EPGs",
          ["Selects all 3 DMZ EPGs",
           "Provider + consumer of Any_VRF-DMZ"],
          accent=GREEN)
...
def design_decisions_slide():
    ...
    rows = [
        ...
        ["L2 'DMZ'-aliased BDs stay in VRF-EUR",
...
def no_ip_change_slide():
    ...
    add_text(s, left_x + Inches(0.15), box_y, left_w, Inches(0.45),
             "Example - BD-APP-SVR (VRF-EUR) absorbs 32 subnets",
             size=13, bold=True, color=WHITE,
             anchor=MSO_ANCHOR.MIDDLE)
    mapping = [
...
def naming_conventions_slide():
    ...
    page_header(s, "Naming & Object Model",
                "Production-ready conventions aligned with the IPv6 RCC model")
    rows = [
        ["VRF (internal)", "VRF-EUR", "VRF-EUR"],
        ["VRF (DMZ)", "VRF-DMZ", "VRF-DMZ"],
        ["VRF (IPv6)", "VRF-RCC (unchanged)", "VRF-RCC"],
        ["Contract", "Any_<VRF>", "Any_VRF-EUR, Any_VRF-DMZ"],
        ["Filter", "Any (ethertype unspecified)", "Any"],
        ["Bridge Domain", "BD-<function>", "BD-DNS-MGMT, BD-UC-DMZ"],
        ["EPG", "EPG-<function>", "EPG-DNS-MGMT, EPG-UC-DMZ"],
        ["App Profile (internal)", "AppProf-NetCentric",
         "AppProf-NetCentric"],
        ["App Profile (DMZ)", "AppProf-DMZ", "AppProf-DMZ"],
        ["App Profile (ESGs)", "AppProf-SecurityGroups",
         "AppProf-SecurityGroups"],
        ["ESG (internal)", "ESG-All-Internal-EPGs",
         "ESG-All-Internal-EPGs"],
    ...
    panel(
        ...
        "Legacy → Redesign",
        [
            "BD-V0172 / V0222 / V0322  →  BD-AD",
            "BD-V0372 + 38 more  →  BD-APP-SVR",
            "BD-V0016 + 39 more  →  BD-CFG-MGMT",
            "BD-V0250 + 10 more  →  BD-DB-SVR",
            "",
            "Each new BD inherits every IPv4 subnet from the legacy BDs it replaces (per docs/reports/bd_mapping_analysis.txt).",
        ],
        accent=ACCENT,
    )
    slides.append(s)
...
def lab_phases_slide():
    ...
    rows = [
        ["1", "Base build",
         "2 VRFs, 39 BDs with consolidated IPv4 subnets, 39 EPGs, VMM domain, vzAny",
         "✓ Complete"],
        ["2", "ESG grouping",
         "36 internal + 3 DMZ EPGs grouped under ESG-All-Internal-EPGs and ESG-All-DMZ-EPGs",
         "✓ Complete"],
        ["3", "Zone ESGs",
         "Split into ESG-AIM, ESG-AIS, ESG-DMZ-Apps, ... with inter-ESG contracts",
         "Future"],
        ["4", "Contract tightening",
         "Restrict flows to what apps actually need", "Future"],
    ]
...
def production_phases_slide():
    ...
    rows = [
        ["1", "Create VRF-EUR and VRF-DMZ alongside existing 11 VRFs",
         "None - new VRFs are empty"],
        ["2", "Create new BDs with descriptive names in VRF-EUR/VRF-DMZ",
         "None - new BDs have no endpoints yet"],
        ["3", "Create ESGs and vzAny contracts on VRF-EUR/VRF-DMZ",
         "None - policy ready, no enforcement yet"],
        ["4", "Migrate EPGs one-at-a-time from old VRFs to new VRFs",
        ...
        ["6", "Rename EPGs/BDs from numeric to descriptive",
         "Cosmetic only - can be done during or after"],
        ["7", "Decommission old VRFs, contracts, and L3Outs",
...
def deployed_objects_slide():
    ...
    rows_tenant = [
        ["Filter", "Any (ethertype unspecified)"],
        ["Contract", "Any_VRF-EUR (scope: context)"],
        ["Contract", "Any_VRF-DMZ (scope: context)"],
        ["VRF-EUR", "vzAny provider + consumer of Any_VRF-EUR"],
        ["VRF-DMZ", "vzAny provider + consumer of Any_VRF-DMZ"],
        ["Bridge Domains (internal)",
         "36 BDs - multi-subnet from legacy consolidation"],
        ["Bridge Domains (DMZ)",
...
def ndo_operator_workflow_slide():
    ...
    steps = [
        ...
        ("2", "Apply schema (NDO only, no APIC push)",
         "make plan && make apply\n"
         "# Creates schema AEDCE-IPv4 / template Tenant_EUR_IPv4\n"
         "# 2 VRFs, 2 contracts, 39 BDs, 2 ANPs, 39 EPGs in NDO\n"
         "# deploy_templates=false -> nothing on APICs yet"),
        ("3", "Deploy from NDO UI (manual click)",
         "Schemas -> AEDCE-IPv4 -> Tenant_EUR_IPv4 -> Deploy to sites\n"
...
def next_steps_slide():
    ...
    columns = [
        ("Lab", GREEN,
         [
             "Phase 1 complete: NDO schema AEDCE-IPv4 / single template "
             "Tenant_EUR_IPv4 deployed via apic-vmware/ + ndo/.",
             "Push static port bindings via scripts/deploy_bindings.py "
             "and re-deploy from NDO UI.",
             "Phase 2 (ESGs) deferred until nac-ndo exposes ESG selectors.",
             "Document membership rules ahead of Phase 3 (zone ESGs).",
         ]),
