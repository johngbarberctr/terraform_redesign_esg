...
FABRIC="${1:-kelley}"
case "$FABRIC" in
  kelley) VMM_DOMAIN_NAME="Kelley-VDS1" ;;
  deldin) VMM_DOMAIN_NAME="Del-Din-VDS1" ;;
  *)
    echo "render-vmm-yaml: unknown fabric '$FABRIC'. Use one of: kelley, deldin." >&2
    exit 2
    ;;
esac
# VMM_DOMAIN_NAME is the VMware VMM domain / VDS name created on each fabric's
# APIC. Kelley -> Kelley-VDS1, Del-Din -> Del-Din-VDS1. Each fabric gets its
# own VDS in vCenter.
