...
# EPG-LB-V2, EPG-LMR-V2, EPG-VHOST-MGMT-V2) without parsing YAML structure.
#
...
def parse_epgs(schema_yaml_path: str) -> list[dict[str, Any]]:
    ...
    ESGs (no EPGs), so this schema parser never sees it. If you ever
    switch to a multi-template schema this will need a YAML parser
    instead.
    """
...
def main() -> int:
    ...
    p.add_argument(
        ...
        action="store_true",
        help=(
            "Skip the three DMZ EPGs (EPG-D64-PROXY-V2, EPG-FWEB-PROXY-V2, "
            "EPG-RWEB-PROXY-V2). Useful if those terminate behind a firewall, "
            "not the FIs."
        ),
    )
    p.add_argument(
        ...
        help=(
            ...
            "(Approach 2: VMM + static together on the FI uplinks)."
        ),
    )
    p.add_argument(
        "--output-manifest",
